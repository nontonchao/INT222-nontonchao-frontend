      ...
      <section class="border bottom-dark" style="background: #ffffff;">
        <nav class="navbar navbar-light navbar-expand-md py-3" style="margin: 2px;">
          <div class="container"><a class="navbar-brand d-flex align-items-center" href="#"><span
                class="fw-bold">นัดหมายทั้งหมด</span></a>
            <div class="collapse navbar-collapse" id="navcol-2">
              <ul class="navbar-nav ms-auto" v-show="eventList.length > 0">
                ...
                <li class="nav-item"><select class="form-select form-select-sm mt-1">
                    <option selected>ทั้งหมด</option>
                    <option v-for="(eCatelist, index) in eventCateList" :key="index">{{ eCatelist.eventCategoryName }}
                    </option>
                  </select></li>
              </ul>
            </div>
      ...
      <div class="container">
        <div v-if="eventList.length <= 0">
          <div class="text-center p-4 p-lg-5">
            <h1 class="fw-bold mb-4 display-5" style="margin: 125px;">No Scheduled Events</h1>
            <figure style="margin: 0px;padding: 45px;">
              <blockquote class="blockquote">
                <p class="mb-0 display-6">ยังไม่มีนัดหมายเลย ลองจองมั้ยล่ะ?</p>
              </blockquote>
              <figcaption class="text-muted" style="font-size: 20px;">จองนัดหมายแล้วไปกันเลย!</figcaption>
            </figure><button class="btn btn-danger fs-5 me-2 py-2 px-4" type="button"
              style="margin: -57px 0px;--bs-primary-rgb: 220,53,69;padding: 0px 24px;">
        ...
        <div v-else>
          <div class="text-center text-white-50 bg-danger border rounded border-0 p-3 my-5">
            <div class="row row-cols-2 row-cols-md-4">
              <div class="col">
                <div class="p-3">
                  <h4 class="display-5 fw-bold text-white mb-0">{{ eventList.length }}</h4>
